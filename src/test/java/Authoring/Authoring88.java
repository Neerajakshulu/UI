...
public class Authoring88 extends TestBase {
	...
	public void beforeTest() throws Exception {
		...
		test = extent.startTest(var, "Verify that spam check and unsupported tag validations are not done on draft post"+
		"and Verify that spam check and unsupported tag validations are done when user publishes the draft post").assignCategory(
				"Authoring");
		...
		try {
			...
			try {
				...
				test.log(LogStatus.FAIL,
						"Spam check is applied on the post and post is not saved as draft");
			...
			jsClick(ob,ob.findElement(By.cssSelector(OnePObjectMap.HOME_PROJECT_NEON_PROFILE_DRAFT_POST_FIRST_TITLE_CSS.toString())));
			...
			try{
				...
				test.log(LogStatus.PASS,
						"Spam check is applied on the draft post while publishing it");
				pf.getProfilePageInstance(ob).clickOnPostCancelButton();
				pf.getProfilePageInstance(ob).clickOnPostCancelDiscardButton();
				
			}catch (Throwable t) {
				test.log(LogStatus.FAIL,
						"Spam check is not applied on the draft post while publishing it");
				test.log(LogStatus.INFO, "Error--->" + t);
				ErrorUtil.addVerificationFailure(t);
				status = 2;
				test.log(
