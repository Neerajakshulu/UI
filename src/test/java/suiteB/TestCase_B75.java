...
public class TestCase_B75 extends TestBase {
	static int status = 1;
...
//			ob.navigate().to(CONFIG.getProperty("testSiteName"));
			...
			if(!compareStrings(expected_text,actual_text)){
				...
				test.log(LogStatus.FAIL, "Record view page of a patent does not get displayed when user clicks on any patent option in the search type ahead while PATENTS option is selected in the search drop down");// extent report
            	status = 2;// excel
            	test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(
            			captureScreenshot(this.getClass().getSimpleName() + "_record_view_page_of_a_patent_not_getting_displayed")));// screenshot
