	...
	public void beforeTest() {
		...
		test = extent
				.startTest(this.getClass().getSimpleName(),
						"To verify that 10 MORE button is present in search results page if total search results is more than 10")
	...
	public void testcaseB17() throws Exception {
		...
		try {
			...
			if (!checkElementIsDisplayed(ob, By.cssSelector(OR.getProperty("tr_search_results_more_button_css")))) {
				...
				test.log(LogStatus.PASS, "More link is displayed for results more than 10");
			}
			logout();
			closeBrowser();
		}
			...
			test.log(LogStatus.FAIL, "Search is not returning more than 10 results.");// extent
																						// reports
