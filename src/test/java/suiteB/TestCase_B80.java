...
public class TestCase_B80 extends TestBase{
	static int status=1;
...
//      3--->SKIP
	...
	public void beforeTest() throws Exception{
		...
		test = extent.startTest(var, "Verify that following options get displayed in SORT BY drop down in ALL search results page:a)Relevanceb)Times Citedc)Date").assignCategory("Suite B");
...
//			System.out.println(master_cond);
			...
			try{
				...
				test.log(LogStatus.PASS,"Correct sorting options present in SORT BY drop down in ALL search results page");//extent reports
			}
			
			
			catch(Throwable t){
				
				test.log(LogStatus.FAIL,"Correct sorting options not present in SORT BY drop down in ALL search results page");//extent reports
				status=2;//excel
				test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(captureScreenshot(this.getClass().getSimpleName()+"_correct_sorting_options_not_present_in_SORT_BY_drop_down_in_ALL_search_results_page")));//screenshot
