...
public class TestCase_B71 extends TestBase {
	static int status = 1;
	...
	public void testcaseB71() throws Exception {
		...
		try {
			...
			if(!compareStrings(expected_text,actual_text)){
				...
				test.log(LogStatus.FAIL, "Record view of a patent does not get displayed when user clicks on any patent option in the search type ahead while ALL option is selected in the search drop down");// extent report
            	status = 2;// excel
            	test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(
            			captureScreenshot(this.getClass().getSimpleName() + "_record_view_of_a_patent_not_getting_displayed")));// screenshot
