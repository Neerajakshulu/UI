...
public class TestCase_B74 extends TestBase {
	static int status = 1;
	...
	public void beforeTest() throws Exception{ extent = ExtentManager.getReporter(filePath);
		...
		test = extent
				.startTest(var,
						"Verify that record view of an article gets displayed when user clicks on any article option in the search type ahead while ARTICLES option is selected in the search drop down")
...
//			ob.navigate().to(CONFIG.getProperty("testSiteName"));
			...
			waitForElementTobeClickable(ob, By.xpath("//a[contains(text(),'Articles')]"), 120);
			...
			if(!compareStrings(expected_text,actual_text)){
				...
				test.log(LogStatus.FAIL, "Record view page of an article does not get displayed when user clicks on any article option in the search type ahead while ARTICLES option is selected in the search drop down");// extent report
            	status = 2;// excel
            	test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(
            			captureScreenshot(this.getClass().getSimpleName() + "_record_view_page_of_an_article_not_getting_displayed")));// screenshot
