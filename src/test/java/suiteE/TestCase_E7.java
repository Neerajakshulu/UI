...
public class TestCase_E7 extends TestBase {
	...
	public void unWatchArticleSearchScreen(String userName, String password) throws Exception {
		...
		try {
			...
			// List<WebElement>
			// watchLists=ob.findElements(By.xpath("//i[@class='webui-icon
			// webui-icon-watch cursor-pointer watch-icon-inactive']"));
			...
			// get the count of the total no.of articles present in watchlist
			// page
			watchListCountBeforeDelete = BrowserAction
					.getElement(OnePObjectMap.HOME_PROJECT_NEON_WATCHLIST_ARTICLE_COUNT_CSS).getText();
			...
			if (mylist.size() > 0) {
				ob.findElement(By.xpath("//i[@class='webui-icon webui-icon-watch watch-icon-active cursor-pointer']"))
						.click();
				Thread.sleep(2000);
			...
			watchListCountAfterDelete = BrowserAction
					.getElement(OnePObjectMap.HOME_PROJECT_NEON_WATCHLIST_ARTICLE_COUNT_CSS).getText();
			...
			test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(
					captureScreenshot(this.getClass().getSimpleName() + "   something unexpected Error")));// screenshot
			closeBrowser();
		}
