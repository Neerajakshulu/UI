...
public class DRAIAMSSO003 extends TestBase{
	static int count = -1;
	...
	/**
	 * Verify Error message when User is associated with one or more active 
	 * UNP subscription(s) across other Neon Sub Apps.
	 * @throws Exception , When TR Login is not done
	 */
  @Test
  public void testcaseDRASSO3() throws Exception {
	  boolean testRunmode = getTestRunMode(rowData.getTestcaseRunmode());
        ...
        String actualErrorMessage="Please sign in to Drug Research Advisor with the email address and password used to create your Clarivate Analytics account.";
		...
		try {
			...
			pf.getDraSSOPageInstance(ob).loginDRAApp(LOGIN.getProperty("DRASSOSUBFLW003"),LOGIN.getProperty("DRASSOPASSWORD"));
			//Verify Error message when user is associated with more active UNP Subscription.
			try{
			waitForElementTobePresent(ob, By.xpath(OnePObjectMap.DRA_SSO_LOGIN_PAGE_SUBFLOW3_ERRORMESSAGE.toString()), 60);
			waitForElementTobePresent(ob, By.xpath(OnePObjectMap.DRA_SSO_LOGIN_PAGE_QUESTIONCONTACT.toString()), 60);
			String expectedErrorMsg=ob.findElement(By.xpath(OnePObjectMap.DRA_SSO_LOGIN_PAGE_SUBFLOW3_ERRORMESSAGE.toString())).getText();
