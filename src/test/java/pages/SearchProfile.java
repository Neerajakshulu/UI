...
public class SearchProfile  extends TestBase {
	...
	public int getPeopleCount() throws Exception {
			String listPeople=pf.getBrowserActionInstance(ob).getElements(OnePObjectMap.HOME_PROJECT_NEON_SEARCH_PEOPLE_CSS).get(2).findElement(By.tagName("span")).getText();
	...
	public void hcrProfileBadgeValidation() throws Exception {
		pf.getBrowserWaitsInstance(ob).waitUntilElementIsDisplayed(OnePObjectMap.HOME_PROJECT_NEON_PROFILE_HCR_BADGE_CSS);
			String hcrAttr=pf.getBrowserActionInstance(ob).getElement(OnePObjectMap.HOME_PROJECT_NEON_PROFILE_HCR_BADGE_CSS).findElement(By.tagName("span")).getAttribute("class");
