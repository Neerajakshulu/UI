...
public class Authoring10 extends TestBase {
	...
	public void beforeTest() throws Exception {
		...
		test = extent.startTest(var, "Verify  that user can not add unsupported html tags while adding the comments")
	...
	public void unSupportedTagsCommentsCheck(String htmlTags,
		...
		try {
			...
			test.log(
					...
					"Snapshot below: "
							+ test.addScreenCapture(captureScreenshot(this.getClass().getSimpleName()
									+ "_UnSupported_HTML_tags_doesnot_allow_comments_validation")));
			// closeBrowser();
		} finally {
			reportDataSetResult();
			++count;
	...
	public void reportDataSetResult() {
		/*
		 * if(skip) { TestUtil.reportDataSetResult(suiteCxls, this.getClass().getSimpleName(), count+2, "SKIP"); } else
		 * if(fail) { status=2; TestUtil.reportDataSetResult(suiteCxls, this.getClass().getSimpleName(), count+2,
		 * "FAIL"); } else { TestUtil.reportDataSetResult(suiteCxls, this.getClass().getSimpleName(), count+2, "PASS");
		 * }
		 */
	...
	public void reportTestResult() {
		...
		/*
		 * if(status==1) TestUtil.reportDataSetResult(suiteCxls, "Test Cases",
		 * TestUtil.getRowNum(suiteCxls,this.getClass().getSimpleName()), "PASS"); else if(status==2)
		 * TestUtil.reportDataSetResult(suiteCxls, "Test Cases",
		 * TestUtil.getRowNum(suiteCxls,this.getClass().getSimpleName()), "FAIL"); else
		 * TestUtil.reportDataSetResult(suiteCxls, "Test Cases",
