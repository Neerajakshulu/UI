...
/**
 * Class for Verifying that Symbol (-) works for both LN&FN
 * 
 * @author UC225218
 *
 */
public class WAT86 extends TestBase {
	...
	/**
	 * Method to Verify that Symbol (-) works for both LN&FN
	 * 
	...
	public void testRemoneAltNameBothLNFN(String LastName, String FirstName)
			throws Exception {
		...
		try {
			...
			List<WebElement> ele = pf.getBrowserActionInstance(ob).getElements(OnePObjectMap.WAT_REMOVE_ALT_NAME_BTN_TEXT_XPATH);
			if (ele.size() == 0)
			{
				test.log(LogStatus.PASS,"Remove Alternate button is not displayed when there is only one name in Author search page.");
			}
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
