...
public class WAT120 extends TestBase {
	...
	public void testSortingInAuthorSearchResultPage(String LastName, String CountryName1,String CountryName2, String OrgName1,String OrgName2)
		...
		try {
			...
			Thread.sleep(3000);
			System.out.println(pf.getBrowserActionInstance(ob)
					.getElement(OnePObjectMap.WAT_SORT_BY_RELEVANCE_XPATH).getText());
			Assert.assertEquals(pf.getBrowserActionInstance(ob)
					.getElement(OnePObjectMap.WAT_SORT_BY_RELEVANCE_XPATH).getText(),"Relevance",
					"Sort by Relavance is not dispalyed in Author Search result page");
			test.log(LogStatus.PASS, "Sort by Relavance is dispalyed in Author Search result page");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Sort by Relavance is not dispalyed in Author Search result page", "search_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
