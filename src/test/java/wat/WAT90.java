...
/**
 * Class for Verify that user should able to access feedback survey page using 'Tell us what you think' link within the informational text on the author results page
 * 
 * @author UC225218
 *
 */
public class WAT90 extends TestBase {
	...
	/**
	 * Method to Verify that user should able to access feedback survey page using 'Tell us what you think' link within the informational text on the author results page
	 * 
	 * @param LastName
	 * @throws Exception,
	 *             When Something unexpected
	...
	public void testFeedbackLink(String LastName, String CountryName1,String CountryName2, String OrgName1,String OrgName2)
		...
		try {
			...
			Assert.assertTrue(pf.getBrowserActionInstance(ob).getElement(OnePObjectMap.WAT_FEEDBACK_AUTHOR_SEARCH_RESULT_PAGE_XPATH).isDisplayed(),"Feedback Link not available");
			test.log(LogStatus.PASS,"Feed back link available");
			pf.getBrowserActionInstance(ob).getElement(OnePObjectMap.WAT_FEEDBACK_AUTHOR_SEARCH_RESULT_PAGE_XPATH).click();
			...
			test.log(LogStatus.PASS,"Feed back link available and navigated to feedback page successfully from author search result page");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
