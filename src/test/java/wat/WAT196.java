...
/**
 * Class to Verify that remove Publication Bulk panel options are not shown in
 * non-curation mode.
 * 
 * @author UC225218
 *
 */
public class WAT196 extends TestBase {
	...
	/**
	 * Method to Verify that remove Publication Bulk panel options are not shown
	 * in non-curation mode.
	 * 
	 * @param LastName,
	...
	public void testCurationBulkRemoval() throws Exception {
		try {
			...
			test.log(LogStatus.INFO,
					"Checking whether Publication bulk removal options are available before getting into curation mode in Author record page");
			if(pf.getBrowserActionInstance(ob)
				...
				throw new Exception("Publication bulk removal options are available before getting into curation mode in Author record page");
			test.log(LogStatus.PASS,
					"Publication bulk removal options are not available before getting into curation mode in Author record page");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Publication bulk removal options are available before getting into curation mode in Author record page",
					"Curation_issue");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
