...
/**
 * Class to Verify that publication list has no "X" mark in non curation mode
 * 
 * @author UC225218
 *
 */
public class WAT197 extends TestBase {
	...
	/**
	 * Method to Verify that publication list has no "X" mark in non curation mode
	 * 
	 * @param LastName,
	...
	public void testCurationPublicationRemoval() throws Exception {
		try {
			...
			test.log(LogStatus.INFO,
					"Checking whether each Publication removal options are available before getting into curation mode in Author record page");
			if(pf.getBrowserActionInstance(ob)
				...
				throw new Exception("Each publication removal options are available before getting into curation mode in Author record page");
			test.log(LogStatus.PASS,
					"Each publication removal options are not available before getting into curation mode in Author record page");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Each publication removal options are available before getting into curation mode in Author record page",
					"Curation_issue");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
