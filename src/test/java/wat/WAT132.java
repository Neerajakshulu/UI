...
/**
 * Class for Verify that divider or spacing is present to visually separate selected options
 * @author UC225218
 *
 */
public class WAT132 extends TestBase {
	...
	/**
	 * Verify that divider or spacing is present to visually separate selected options
	 * 
	 * @param LastName,
	...
	public void testDividerLineBetweenFilter(String LastName, String CountryName1, String CountryName2, String OrgName1,
		...
		try {
			...
			if (pf.getBrowserActionInstance(ob).getElement(OnePObjectMap.WAT_SEARCH_RESULTS_FILTER_NAME_XPATH)
				...
				throw new Exception(
						"Divider line for Author Name filter seperating filtered and unfiltered names is not displayed");
			}

		} catch (Throwable t) {
			logFailureDetails(test, t, "Divider line for Author name filter is not displayed", "Filter_line_issue");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
