...
/**
 * Class for Verify that tab name for ORCiD search will be "ORCiD search"
 * 
 * @author UC225218
 *
 */

public class WAT110 extends TestBase {
	...
	/**
	 * Method to Verify that tab name for ORCiD search will be "ORCiD search"
	 * 
	 * @throws Exception,
	 *             When Something unexpected
	 * 
	...
	public void testOrcidButton() throws Exception {

		// NOTE - Assertion ERRORs are caught in EXCEPTION block just for
		// Reporting purpose
		try {
				Assert.assertEquals(pf.getBrowserActionInstance(ob)
						.getElement(OnePObjectMap.WAT_ORCID_SEARCH_BTN_XPATH).getText(),Orcid_button_text,
						"Orcid button is not displayed");
				test.log(LogStatus.PASS, "Orcid button is displayed");
				pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Exception e) {
				test.log(LogStatus.FAIL, "Orcid button is not displayed");
				logFailureDetails(test, e, "Orcid button is not displayed", "Orcid_btn_error");
				pf.getBrowserActionInstance(ob).closeBrowser();
		}
	}
