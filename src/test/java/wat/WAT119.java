...
public class WAT119 extends TestBase {
	...
	public void testAddAltNameLink()
			throws Exception {
			try {
				Assert.assertTrue(pf.getBrowserActionInstance(ob)
						.getElement(OnePObjectMap.WAT_ADD_ALT_NAME_BTN_TEXT_XPATH).isDisplayed(),
						"Add alternate name link is not displayed or shown as Include alternative name");
				Assert.assertEquals(pf.getBrowserActionInstance(ob)
						.getElement(OnePObjectMap.WAT_ADD_ALT_NAME_BTN_TEXT_XPATH).getText(),"Include alternative name");
				test.log(LogStatus.PASS, "Add alternate name link is displayed and shown as Include alternative name");
				pf.getBrowserActionInstance(ob).closeBrowser();
			} catch (Throwable t) {
				logFailureDetails(test, t, "Add alternate name link is not displayed or shown as Include alternative name", "Add_alt_name_text_error");
				pf.getBrowserActionInstance(ob).closeBrowser();
			}
