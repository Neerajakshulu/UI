...
/**
 * Class to Verify that System must display a maximum of 3 recommendations on
 * the author record at a time(For Combined Author) individual pubs options are
 * also displayed."Suggest updates" is hidden.
 * 
 * @author UC225218
 *
 */
public class WAT191 extends TestBase {
	...
	/**
	 * Method to Verify that System must display a maximum of 3 recommendations
	 * on the author record at a time(For Combined Author)
	 * @param LastName,
	...
	public void testRecommendationCountCombinedAuthor() throws Exception {
		try {
			...
			List<WebElement> lis = pf.getBrowserActionInstance(ob).getElements(OnePObjectMap.WAT_AUTHOR_RECORD_RECOMMEND_PUBLICATIONS_XPATH);
			if(lis.size() >3 || lis.size() <3)
				throw new Exception("Recommendation publication count is more than the expected count (i.e 3)");
			test.log(LogStatus.PASS, "Maximum of 3 recommendation publication are displayed ");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Recommendation publication count is more than the expected count (i.e 3)", "Curation_issue");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
