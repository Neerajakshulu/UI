...
/**
 * Class for Verify that ResearcherID format (text) label followed by the ResearcherID number i.e. ResearcherID A-1009-2008
 * 
 * @author UC225218
 *
 */

public class WAT104 extends TestBase {

	static int status = 1;
	private static final String ResearcherID_MATCHER_PATTERN = "[A-Z]-[0-9]{4}-[0-9]{4}";
	...
	/**
	 * Method to Verify that ResearcherID format (text) label followed by the ResearcherID number i.e. ResearcherID A-1009-2008
	 * 
	 * @throws Exception,
	 *             When Something unexpected
	 * 
	...
	public void testResearcherIDFormat(String LastName, String CountryName1,String CountryName2, String OrgName1,String OrgName2) throws Exception {
		try {
		    ...
		    Pattern pattern = Pattern.compile(ResearcherID_MATCHER_PATTERN, Pattern.CASE_INSENSITIVE);
		   
		    Matcher matcher = pattern.matcher(RESEARCHERID);
		    if (matcher.matches()) {
		    	test.log(LogStatus.PASS, "Researcher ID match found " + matcher.group()
		             + " starting at index " + matcher.start()
		             + " and ending at index " + matcher.end());
		    } else {
		    	Assert.assertTrue(false);
		    	test.log(LogStatus.FAIL, "Researcher ID match not found");
		    }
		    pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (AssertionError e) {
				test.log(LogStatus.FAIL, "Researcher ID match not found");
				logFailureDetails(test, e, "Researcher ID match not found", "ResearcherID_match_fail");
				pf.getBrowserActionInstance(ob).closeBrowser();
			}
	}
