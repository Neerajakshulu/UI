...
/**
 * Class for Verify that the user should be able to further refine the search result based on Author name
 * 
 * @author UC225218
 *
 */

public class WAT101 extends TestBase {
	...
	public void testFilterInAuthorSearchResultPageFunctionality(String LastName, String CountryName1,String CountryName2, String OrgName1,String OrgName2) throws Exception {
		try {
			...
			pf.getSearchAuthClusterResultsPage(ob).verifyNameFilterFunctionality(test);
		} catch (AssertionError e) {
				test.log(LogStatus.FAIL, "User is not able to further refine the search result based on Author name");
				logFailureDetails(test, e, "User is not able to further refine the search result based on Author name", "Name_filter_fail");
				pf.getBrowserActionInstance(ob).closeBrowser();
			}
	}
