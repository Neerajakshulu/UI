...
/**
 * Class for Verify that user is able to see the selections they made in the Author filter in Author search results page
 * 
 * @author UC225218
 *
 */
public class WAT126 extends TestBase {
	...
	/**
	 * Method to Verify that user is able to see the selections they made in the Author filter in Author search results page
	 * 
	 * @param LastName,
	...
	public void testSearchAuthorClusterOnlyLastName(String LastName, String CountryName1,String CountryName2, String OrgName1,String OrgName2)
		...
		try {
			...
			if(pf.getBrowserActionInstance(ob).getElement(
					OnePObjectMap.WAT_SEARCH_RESULTS_FILTER_NAME_XPATH).isDisplayed() && 
					pf.getBrowserActionInstance(ob).getElement(
							OnePObjectMap.WAT_SEARCH_RESULTS_FILTER_OPTIONS_NAME_XPATH).isDisplayed())
			{
				...
				Assert.assertTrue(pf.getBrowserActionInstance(ob).getElement(OnePObjectMap.WAT_AUTHOR_NAME_DIVIDER_XPATH).isDisplayed(),
						"Divider line for Author name seperating filtered and unfiltered names is not displayed");
				test.log(LogStatus.PASS, "Divider line displayed and successfully seperates filtered and unfiltered author names in the author filter in Author search results page");
				pf.getBrowserActionInstance(ob).closeBrowser();
			}else{
				throw new Exception("Divider line for Author name seperating filtered and unfiltered names is not displayed");
			}
