...
public class ENWIAM62 extends TestBase {
	...
	public void testcaseh20() throws Exception {
		...
		try {
			String statuCode = deleteUserAccounts(LOGIN.getProperty("sru_steam15"));
		...
		try {
			...
			try {
				...
				pf.getLoginTRInstance(ob).enterTRCredentials(LOGIN.getProperty("sru_steam15"),
			...
			try {
				...
				try {
					...
					/*
					 * try { pf.getBrowserWaitsInstance(ob).
					 * waitUntilElementIsClickable(OnePObjectMap.
					 * ENW_HOME_AGREE_CSS);
					 * pf.getBrowserActionInstance(ob).jsClick(OnePObjectMap.
					 * ENW_HOME_AGREE_CSS); pf.getBrowserWaitsInstance(ob)
					 * .waitUntilElementIsClickable(OnePObjectMap.
					 * ENW_HOME_CONTINUE_XPATH);
					 * pf.getBrowserActionInstance(ob).jsClick(OnePObjectMap.
					 * ENW_HOME_CONTINUE_XPATH); } catch (Exception e) {
					 * pf.getBrowserWaitsInstance(ob)
					 * .waitUntilElementIsClickable(OnePObjectMap.
					 * ENW_HOME_CONTINUE_XPATH);
					 * pf.getBrowserActionInstance(ob).jsClick(OnePObjectMap.
